...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Developer Data Quality Report Section */}
      <Card className="border-slate-200">
        <CardHeader className="bg-slate-50 border-b border-slate-200">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Badge variant="blue">Developer Mode</Badge>
          </div>
        </CardHeader>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <h4 className="font-bold text-slate-800 text-sm flex items-center gap-1.5">
                  ...
                  Sample Duplicated Names / Variants:
                </h4>
                {report.details.duplicateNames.length > 0 ? (
                  <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-3 max-h-48 overflow-y-auto text-xs font-mono text-slate-700 space-y-1">
                    {report.details.duplicateNames.map((name, idx) => (
                      <div key={idx}>• {name}</div>
                    ))}
                  </div>
                ) : (
                  <div className="text-slate-500 text-xs p-3 bg-slate-50 rounded-xl">
                    No duplicate food names detected.
                  </div>
                )}
              </div>
            </div>
